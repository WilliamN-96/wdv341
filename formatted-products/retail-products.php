...
<head>
    ...
    <style>
        *,:after,:before{-moz-box-sizing:border-box;-webkit-box-sizing:border-box;box-sizing:border-box}body{font:normal 15px/25px 'Open Sans',Arial,Helvetica,sans-serif;color:#444;text-align:left}
        ...
        thead{background:#5965af;color:#fff}tbody tr td,thead td{padding:.5rem .75rem}tbody tr:nth-child(even){background:#efefef}tbody tr td:first-child{padding-left:1.25rem}
    ...
    <section>
        ...
        ?>
           <div class="productBlock">
            ...
            <!-- The productStatus element should only be displayed if there is product_status data in the record -->
            <p class="productStatus"><?php echo $row['product_status'];?>New Item!</p>            
            <p class="productInventory <?php if($row['product_inStock'] <= 10){echo 'productLowInventory';}?>"># In Stock!</p>
            </div> <!--stuff goes here - display blocks of data for each row in the database-->
        <?php 
            }
        ?>
    </section>
